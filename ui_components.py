...
function sendMsg(type, payload) {{
  try {{
    ...
    for (var i = 0; i < inputs.length; i++) {{
      if (inputs[i].getAttribute('aria-label') === 'jtbridge' ||
          inputs[i].placeholder === 'JTBRIDGE_UNIQUE_DO_NOT_USE') {{
...
window.toggleZoom = function(sid) {{
  ...
  if (img.classList.contains('locally-zoomed')) {{
    ...
    document.querySelectorAll('.locally-zoomed').forEach(function(el) {{
