...
def build_fast_grid_html(page_data, flags_mapping, country, page_warnings, rejected_state, cols_per_row):
    ...
    for _, row in page_data.iterrows():
        ...
        if not img_url.startswith("http"): img_url = _NO_IMAGE_SVG
...
.batch-btn,.desel-btn{{padding:7px 14px;background:{O};color:#fff;border:none;border-radius:4px;font-weight:700;font-size:12px;cursor:pointer;}}
.desel-btn{{background:#fff;color:#555;border:1px solid #ccc;}}
.desel-btn:hover,.batch-btn:hover{{opacity:.88;}}
...
.card-img{{width:100%;height:180px;object-fit:contain;border-radius:6px;display:block;opacity:0;transition:opacity .25s ease,transform .2s ease-out,box-shadow .2s ease-out;}}
.card-img.img-loaded{{opacity:1;}}
.card-img.locally-zoomed{{transform:scale(2.3);box-shadow:0 15px 50px rgba(0,0,0,0.6);border:2px solid {O};position:relative;z-index:9999;border-radius:8px;}}
...
.warn-badge{{background:linear-gradient(90deg,#FFC107,#FFEB3B);color:#313133;font-size:9px;font-weight:800;padding:3px 7px;border-radius:10px;animation:fadeIn .3s;}}
...
let selected = window._gridSelected, staged = window._stagedRejections;
...
function loadHighRes(img, hiUrl){{
    if(!hiUrl) return;
    const hi=new Image();hi.src=hiUrl;
    hi.onload=()=>{{img.src=hi.src;img.classList.add('img-loaded');}};
    hi.onerror=()=>{{img.src='{_NO_IMAGE_SVG}';img.classList.add('img-loaded');}};
}}
...
function renderCard(card){{
  ...
  var zoomHtml=`<div class="zoom-btn" onclick="event.stopPropagation();toggleZoom('${{sid}}')"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div>`;
  var overlayHtml='', actHtml='';
  if(isCommitted){{overlayHtml=`<div class="rej-overlay"><div class="rej-badge">{rejected_label}</div><div class="rej-label">${{escapeHtml((COMMITTED[sid]||'').replace(/_/g,' '))}}</div><button class="undo-btn" onclick="event.stopPropagation();undoReject('${{sid}}')">{_t('undo')}</button></div>`;}}
  else if(isStaged){{overlayHtml=`<div class="rej-overlay staged"><div class="rej-badge pending">PENDING</div><div class="rej-label">${{escapeHtml((staged[sid]||'').replace(/_/g,' '))}}</div><button class="undo-btn" onclick="event.stopPropagation();clearStaged('${{sid}}')">{_t('clear_sel')}</button></div>`;}}
  else{{actHtml=`<div class="acts"><button class="act-btn" onclick="event.stopPropagation();stageReject('${{sid}}','REJECT_POOR_IMAGE')">{_t('poor_img')}</button><select class="act-more" onchange="if(this.value){{event.stopPropagation();stageReject('${{sid}}',this.value);this.value=''}}"><option value="">{_t('more_options')}</option><option value="REJECT_WRONG_CAT">{_t('wrong_cat')}</option><option value="REJECT_FAKE">{_t('fake_prod')}</option><option value="REJECT_BRAND">{_t('restr_brand')}</option><option value="REJECT_PROHIBITED">{_t('prohibited')}</option><option value="REJECT_COLOR">{_t('missing_color')}</option><option value="REJECT_WRONG_BRAND">{_t('wrong_brand')}</option></select></div>`;}}
  return `<div class="${{cls}}" id="card-${{sid}}"><div class="card-img-wrap">${{tooltipHtml}}${{priceHtml}}<div class="warn-wrap">${{warnHtml}}</div><img class="card-img" src="${{escapeHtml(card.low_res)}}" onload="loadHighRes(this,'${{escapeHtml(card.img)}}')" onerror="this.src='{_NO_IMAGE_SVG}';this.classList.add('img-loaded');">${{zoomHtml}}${{overlayHtml}}<div class="tick">&#10003;</div></div><div class="meta"><div class="nm" title="${{escapeHtml(card.name)}}">${{shortName}}</div><div class="br">${{escapeHtml(card.brand)}}</div><div class="ct">${{escapeHtml(card.cat)}}</div><div class="sl">${{escapeHtml(card.seller)}}</div></div>${{actHtml}}</div>`;
}}
...
function toggleZoom(sid){{
  const img=document.querySelector('#card-'+sid+' .card-img');if(!img)return;
  if(img.classList.contains('locally-zoomed')){{img.classList.remove('locally-zoomed');return;}}
  document.querySelectorAll('.locally-zoomed').forEach(el=>el.classList.remove('locally-zoomed'));
