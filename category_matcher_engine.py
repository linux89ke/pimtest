...
No tkinter, no file I/O, no UI dependencies.

Usage
─────
    from category_matcher_engine import CategoryMatcherEngine

    engine = CategoryMatcherEngine()
    engine.build_tfidf_index(categories_list)          # call once per session
    kw_map = engine.build_keyword_to_category_mapping()

    category = engine.get_category_with_fallback(
        product_name, kw_map, categories_list
    )

Integration in streamlit_app.py
────────────────────────────────
    Replace check_miscellaneous_category with check_wrong_category
    (defined at the bottom of this file) to get AI-powered wrong-category
    detection using the full 5-priority matching pipeline.
"""
...
class CategoryMatcherEngine:
    ...
    def load_learning_db(self):
        """Load learned product→category corrections from JSON file."""
        if os.path.exists(self._LEARNING_DB_PATH):
            try:
                with open(self._LEARNING_DB_PATH, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                db = {k.lower(): v for k, v in data.items()}
            except Exception:
                db = {}
        else:
            db = {}
        # Merge seed corrections — user corrections in the JSON always win
    ...
    def save_learning_db(self):
        """Persist the learning DB to disk."""
        try:
    ...
    def similarity_match(self, product_name, categories_list):
        ...
        Pipeline:
          1. SGDClassifier domain prediction (if trained on corrections)
          2. Keyword domain routing
          3. sklearn/legacy cosine similarity within top candidate domains
          4. Global cosine similarity fallback
        Returns (best_category, score).
        """
        # ── Candidate domains ──────────────────────────────────────────────
    ...
    def get_category_with_fallback(self, product_name, keyword_mapping,
        ...
        Five-priority matching pipeline called for every product:
          ...
          1. Learning DB   — user corrections are absolute truth
          2. Product map   — exact keyword → category lookup (468 entries)
          3. Rule engine   — existing v2 keyword/scoring engine
          4. sklearn / TF-IDF similarity — domain-routed cosine similarity
          5. Global cosine similarity    — last resort across all categories
        """
        ...
        # If the product name contains strong women's signals AND the final
        # result lands in a clearly wrong domain, override it to Women's Fashion.
        # This is applied AFTER all matching so it acts as a safety net.
        _women_signals = [
        ...
        # Measure word overlap between product name and rule result.
        # A plausible result shares at least one meaningful content word.
        stopwords = {
        ...
        # If product has strong women's signal but result is in a clearly wrong
        # domain, redirect to the correct Women's Fashion category.
        if _has_women_signal:
            ...
            # Also fix Men's Fashion assigned to ladies products
    ...
    def _womens_fashion_fallback(self, product_name, categories_list):
        ...
        that was wrongly routed away from Fashion.  Uses keyword signals
        from the product name to pick the right sub-category.
        """
        pn = product_name.lower()
        ...
        for keywords, target_cat in _WOMENS_RULES:
            if any(kw in pn for kw in keywords):
                # Verify the target exists in categories_list (or find closest)
        ...
        # Final fallback — return the most generic Women's Fashion category available
        for cat in categories_list:
    ...
    def identify_product_type(self, product_name):
        ...
        Ignores: Brand names, marketing phrases, quantities, packaging terms
        """
        ...
        # Words and phrases to ignore (brands, marketing, quantities, packaging)
        ignore_patterns = [
        ...
        # Clean product name by removing ignore patterns
        ...
        # =====================================================================
        # WOMEN'S FASHION & APPAREL - HIGHEST PRIORITY (checked FIRST)
        # =====================================================================
        women_fashion_patterns = [
            # Nightwear / Sleepwear sets (top + short/trouser)
            ...
            # Shorts (women context)
            (r'\bshort\b|\bshorts\b(?!.*circuit|.*cut)', 'Women Shorts'),
        ]
        
        # Check women's fashion patterns FIRST (these take top priority for apparel)
        ...
        # =====================================================================
        # MEN'S FASHION - Checked before generic kitchen/electronics patterns
        # =====================================================================
        ...
        # =====================================================================
        # HOME & OFFICE - FURNITURE, DECOR, LIGHTING
        # =====================================================================
        home_patterns = [
        ...
        # Product type identification patterns (from specific to general)
        product_patterns = [
            # Kitchen Appliances
            ...
            # Kitchen Tools - Enhanced with more chopper/slicer patterns
            ...
            # Cookware - Enhanced with more pot patterns
            ...
            (r'\bpressure\s*cooker\b', 'Pressure Cooker'),
            # Pots - match cookware pots with kitchen context to exclude flower/plant pots
            (r'\bkitchen\s*pot\b|\bcooking\s*pot\b|\bcooking\s*pots\b|\bquality\s*pot\b|\bquality\s*pots\b', 'Pot'),
            # Non-stick pots/cookware - for standalone pot/pan products
            (r'\bnon\s*stick\s*pots\b|\bnonstick\s*pots\b|\bnon\s*stick\s*pot\b|\bnonstick\s*pot\b', 'Non-Stick Pots'),
            (r'\bgranite\s*pots\b|\bgranite\s*pot\b|\bdiecast\s*pots\b|\bcast\s*pots\b', 'Cookware Pots'),
            ...
            # Personal Care - Enhanced with vitamins and supplements
            ...
            # Electronics - Enhanced with phone case, ring light, and better adapter patterns
            ...
            # USB Adapter - distinguish from phone charger
            (r'\badapter\b(?!.*phone|.*charger)|\busb\s*adapter\b|\bpower\s*adapter\b(?!.*phone)', 'USB Adapter'),
            # Phone Case - Enhanced patterns for galaxy, iphone, z flip, z fold
            ...
            # Laundry - Enhanced with washing machine accessories and grippers
            ...
            # Musical Instruments - New section
            ...
            # Grocery & Household Cleaning - New section
            ...
            # Industrial & Scientific - New section
        ...
        # Try to identify product type from specific to general
        for pattern, product_type in product_patterns:
            if re.search(pattern, cleaned_name, re.IGNORECASE):
                return product_type, self._get_context_keywords(product_name, product_type)
        
        # Fallback: Extract main product keywords
    ...
    def precompute_leaf_categories(self, categories_list):
        ...
        # Build a set of all normalized category paths
        categories_lower = {cat.lower().strip() for cat in categories_list}
        
        # Build a set of all possible parent paths
        # A category is a parent if there exists another category that starts with this path + '/'
        parent_paths = set()
        
        for cat_lower in categories_lower:
            # Add all possible parent paths (by removing the last segment)
            parts = cat_lower.split('/')
            for i in range(1, len(parts)):
                parent_path = '/'.join(parts[:i]).strip()
                parent_paths.add(parent_path)
        
        # A category is a leaf if its normalized path is NOT in the parent_paths set
        for cat in categories_list:
    ...
    def get_most_specific_category(self, product_type, context_keywords, categories_list, leaf_categories=None, last_category_parts=None):
        ...
        # Use pre-computed values if available, otherwise compute on the fly
        ...
        # =====================================================================
        # WOMEN'S FASHION TYPE ROUTING - direct mapping for apparel types
        # =====================================================================
        product_type_routing = {
            ...
            # Computing extra
            'laptop': 'Computing / Computers & Accessories',
            ...
            'kids shoes': "Fashion / Kids Fashion / Girls / Shoes",
            ...
            'ram': "Computing / Computer Accessories / Networking Accessories",
        ...
        # Check all product type routings (women's fashion AND all other categories)
        if product_lower in product_type_routing:
            ...
            for cat in categories_list:
                if cat.lower() == target_cat:
                    return cat
            # Find closest partial-path match
            target_parts = [p.strip() for p in target_cat.split('/')]
            best = None
            best_score = 0
            for cat in categories_list:
                cl = cat.lower()
                # Must share the top-level domain
        ...
        # Build category scores based on specificity
        category_scores = {}
        
        # Special handling for cookware products (pot, pots, pan, pans)
        ...
        for cat in categories_list:
            ...
            # Get the last part of the category path (the leaf category name)
            last_part = last_category_parts.get(cat, self.get_last_category_part(cat))
            
            # EXACT MATCH with last part of category path - ULTIMATE PRIORITY
            if last_part and (product_lower == last_part or 
                              product_lower in last_part.replace(' ', '') or 
                              last_part in product_lower.replace(' ', '')):
                score += 200
            
            # Special priority: if last part is exactly "Pots & Pans" AND it's a cookware category
            ...
            # Direct match with product type in category path
            if product_lower in cat_lower:
                score += 100
            
            # Match product with last category part using word boundaries
            ...
            # Special cookware handling - prioritize cookware categories over serveware
            ...
            # Specificity bonus - prefer leaf categories (use pre-computed value)
            is_leaf = leaf_categories.get(cat, True)
            if is_leaf:
                score += 50
            
            # Penalize parent categories more heavily
            ...
            # Category path length bonus - longer paths are more specific
            score += slash_count * 10
            
            # Bonus for categories that contain product-related keywords
    ...
    def get_category_for_product_v2(self, product_name, keyword_mapping, categories_list, leaf_categories=None, last_category_parts=None):
        ...
        # =====================================================================
        # STEP 0: WOMEN'S FASHION FAST PATH - check before anything else
        # These patterns are extremely common and must win over all other logic
        # =====================================================================
        ...
        # Nightwear / Sleepwear (ladies / women / unisex children)
        ...
        # Shapewear / Girdle / Tummy tight / Slimming
        ...
        # Sports Leggings (yoga pants, sport tights)
        ...
        # Ladies Tops (non-set: not nightwear)
        ...
        # Bumper Tight (body-hugging shorts/leggings)
        ...
        # =====================================================================
        # STEP 0c: TRADITIONAL & CULTURAL WEAR FAST PATH
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0c: MEN'S FASHION FAST PATH
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0d: KID'S FASHION FAST PATH
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0f: HOME & OFFICE FURNITURE / APPLIANCES FAST PATH
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0g: HEALTH & BEAUTY FAST PATH (makeup, hair, medical)
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0i: BABY PRODUCTS FAST PATH
        # =====================================================================
        ...
        # =====================================================================
        # STEP 0b: Check for exact product name matches (long keywords)
        # =====================================================================
        ...
        # Enhanced helper function to check if a keyword appears as a standalone word in text
        ...
        # Define product type keywords that should have highest priority
        ...
        # Phase 1: Check product type keywords FIRST
        ...
        # Phase 2: Multi-word keywords that are NOT product types
        ...
        # Phase 3: Single-word keywords that are NOT product types
        ...
        # Step 2: Try to match to most specific category
        if product_type:
        ...
        # Step 3: Fallback to existing priority-based matching
    ...
    def build_keyword_to_category_mapping(self):
        ...
        product_type_mappings = {
            # =====================================================================
            # WOMEN'S FASHION - NEW COMPREHENSIVE MAPPINGS
            # =====================================================================
            ...
            # Priority 1: Specific vitamins and supplements (MOST SPECIFIC - checked first)
            ...
            # Priority 2: Specific kitchen and cookware items
            ...
            # Priority 3: Beauty and personal care
            ...
            # Priority 4: Electronics and accessories
            ...
            # Priority 5: Home and household items
            ...
            # Priority 8: Fashion and accessories
            ...
            # Additional Health & Beauty mappings
            ...
            # Additional Electronics mappings
            ...
            # Additional Automotive mappings
            'motor oil': 'Automobile / Oils & Fluids / Oils / Motor Oils',
            'synthetic oil': 'Automobile / Oils & Fluids / Oils / Motor Oils',
            'engine oil': 'Automobile / Oils & Fluids / Oils / Motor Oils',
            
            # Additional Food & Beverage mappings
            'yogurt': 'Grocery',
            'yoghurt': 'Grocery',
            ...
            'bread toaster': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Toasters',
            'toasting machine': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Toasters',
            
            # Additional Home & Kitchen mappings
            'blender': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Blenders',
            'grinder': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Blenders',
            'humidifier': 'Home & Office / Home & Kitchen / Heating, Cooling & Air Quality / Humidifiers',
            'diffuser': 'Home & Office / Home & Kitchen / Heating, Cooling & Air Quality / Humidifiers',
            'essential oil': 'Home & Office / Home & Kitchen / Heating, Cooling & Air Quality / Humidifiers',
            'air fryer liner': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Air Fryer Accessories',
            'water bottle': 'Home & Office / Home & Kitchen / Kitchen & Dining / Travel & To-Go Drinkware / Water Bottles',
            'clothes hanger': 'Home & Office / Home & Kitchen / Storage & Organization / Clothing & Closet Storage / Clothes Hangers',
            'kettle': 'Home & Office / Home & Kitchen / Kitchen & Dining / Small Appliances / Kettles & Hot Pots',
            'laundry accessory': 'Home & Office / Home & Kitchen / Laundry / Laundry Accessories',
            'face mask': 'Health & Beauty / Medical Supplies & Equipment / Medical Supplies / Face Masks',
            
            # Additional Fashion mappings
            'ankara': "Fashion / Fabrics / Women's Fabric / Fabrics",
            'fabric': "Fashion / Fabrics / Women's Fabric / Fabrics",
            't-shirt': "Fashion / Men's Fashion / Clothing / T-Shirts & Tanks / T-shirts",
            't shirt': "Fashion / Men's Fashion / Clothing / T-Shirts & Tanks / T-shirts",
            'clothing set': "Fashion / Men's Fashion / Clothing",
            'polo': "Fashion / Men's Fashion / Clothing / Shirts / Polos",
            'singlet': "Fashion / Men's Fashion / Clothing / T-Shirts & Tanks / Tanks",
            'shoes': "Fashion / Men's Fashion / Shoes / Fashion Sneakers",
            'sneakers': "Fashion / Men's Fashion / Shoes / Fashion Sneakers",
            'loafers': "Fashion / Womens Fashion / Shoes / Flats",
            'flats': "Fashion / Womens Fashion / Shoes / Flats",
            'wrist watch': "Fashion / Men's Fashion / Watches / Wrist Watches",
            'watch set': "Fashion / Men's Fashion / Watches / Wrist Watches",
            'watch strap': "Fashion / Men's Fashion / Watches / Watch Bands",
            'watch band': "Fashion / Men's Fashion / Watches / Watch Bands",
            'waist pack': "Fashion / Luggage & Travel Gear / Waist Packs",
            'necklace': "Fashion / Womens Fashion / Jewelry / Necklaces",
            'gown': "Fashion / Womens Fashion / Dresses / Gowns",
            'vest': "Fashion / Men's Fashion / Clothing / T-Shirts & Tanks / Vests",
            
            # Additional Arts & Crafts mappings
            'yarn': 'Home & Office / Arts, Crafts & Sewing / Knitting & Crochet / Yarn',
            'knitting yarn': 'Home & Office / Arts, Crafts & Sewing / Knitting & Crochet / Yarn',
            'crochet yarn': 'Home & Office / Arts, Crafts & Sewing / Knitting & Crochet / Yarn',
            'stickers': 'Toys & Games / Arts & Crafts / Stickers',
            'decal': 'Toys & Games / Arts & Crafts / Stickers',
            'cutting mat': 'Home & Office / Arts, Crafts & Sewing / Scrapbooking & Stamping / Cutting Mats',
            'crystal': 'Home & Office / Arts, Crafts & Sewing / Painting, Drawing & Art Supplies / Drawing / Art Blenders',
            'gemstone': 'Home & Office / Arts, Crafts & Sewing / Painting, Drawing & Art Supplies / Drawing / Art Blenders',
            
            # Additional Toys & Games mappings
            'bubble gun': 'Toys & Games / Sports & Outdoor Play / Bubbles',
            'bubble': 'Toys & Games / Sports & Outdoor Play / Bubbles',
            
            # =====================================================================
            # MEN'S FASHION - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'agbada': "Fashion / Traditional & Cultural Wear / African",
            'senator': "Fashion / Traditional & Cultural Wear / African",
            'daishiki': "Fashion / Traditional & Cultural Wear / African",
            'men jeans': "Fashion / Men's Fashion / Clothing / Jeans",
            'men shorts': "Fashion / Men's Fashion / Clothing / Shorts",
            'men shirt': "Fashion / Men's Fashion / Clothing / Shirts",
            'men trouser': "Fashion / Men's Fashion / Clothing / Pants",
            'men suit': "Fashion / Men's Fashion / Clothing / Suits",
            'men jacket': "Fashion / Men's Fashion / Clothing / Jackets & Coats",
            'men hoodie': "Fashion / Men's Fashion / Clothing / Fashion Hoodies & Sweatshirts",
            'men cap': "Fashion / Men's Fashion / Accessories / Hats & Caps",
            ...
            'men socks': "Fashion / Men's Fashion / Accessories / Socks",
            'men watch': "Fashion / Watches & Sunglasses / Men's Watches",
            'men shoe': "Fashion / Men's Fashion / Shoes",
            # =====================================================================
            # ELECTRONICS - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'smart tv': "Electronics / Television & Video / Televisions",
            'led tv': "Electronics / Television & Video / Televisions",
            'oled tv': "Electronics / Television & Video / Televisions",
            '4k tv': "Electronics / Television & Video / Televisions",
            'flat screen': "Electronics / Television & Video / Televisions",
            'television': "Electronics / Television & Video / Televisions",
            'soundbar': "Electronics / Home Audio / Home Theater Systems / Sound Bars",
            'sound bar': "Electronics / Home Audio / Home Theater Systems / Sound Bars",
            'home theater': "Electronics / Home Audio / Home Theater Systems",
            'bluetooth speaker': "Electronics / Home Audio / Speakers / Bluetooth Speakers",
            'portable speaker': "Electronics / Home Audio / Speakers / Bluetooth Speakers",
            'earphone': "Electronics / Portable Audio & Video / Headphones",
            'earbud': "Electronics / Portable Audio & Video / Headphones",
            'wireless earbud': "Electronics / Portable Audio & Video / Headphones",
            'tws': "Electronics / Portable Audio & Video / Headphones",
            'security camera': "Electronics / Cameras / Security & Surveillance Cameras",
            'cctv': "Electronics / Cameras / Security & Surveillance Cameras",
            ...
            'action camera': "Electronics / Cameras / Video Cameras",
            # =====================================================================
            # HOME & OFFICE FURNITURE - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'sofa': "Home & Office / Home & Furniture / Sofas & Armchairs",
            ...
            'laminator': "Home & Office / Office Products / Office Supplies & Equipment",
            # =====================================================================
            # HEALTH & BEAUTY - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'hair extension': "Health & Beauty / Beauty & Personal Care / Hair Care / Extensions, Wigs & Accessories",
            ...
            # =====================================================================
            # BABY PRODUCTS - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'baby formula': "Baby Products / Feeding / Formula",
            'infant formula': "Baby Products / Feeding / Formula",
            'baby food': "Baby Products / Feeding",
            ...
            # =====================================================================
            # PHONES & TABLETS - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'smartphone': "Phones & Tablets / Mobile Phones / Smartphones",
            ...
            # =====================================================================
            # COMPUTING - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'laptop': "Computing / Computers & Accessories / Laptops",
            ...
            # =====================================================================
            # AUTOMOBILE - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            'car wash': "Automobile / Car Care / Exterior Care / Car Wash Equipment",
            ...
            # =====================================================================
            # MENSTRUAL, SANITARY AND FEMININE CARE
            # =====================================================================
            'sanitary pad': "Health & Beauty / Beauty & Personal Care / Personal Care / Feminine Hygiene",
            'menstrual pad': "Health & Beauty / Beauty & Personal Care / Personal Care / Feminine Hygiene",
            ...
            # =====================================================================
            # INDUSTRIAL & SCIENTIFIC - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            ...
            # =====================================================================
            # WOMEN SHOES - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            ...
            # =====================================================================
            # WOMEN'S ACCESSORIES - ADDITIONAL KEYWORD MAPPINGS
            # =====================================================================
            ...
            # =====================================================================
            # MUSICAL INSTRUMENTS - NEW COMPREHENSIVE MAPPINGS
            # =====================================================================
            ...
            # =====================================================================
            # GAMING - NEW COMPREHENSIVE MAPPINGS
            # =====================================================================
            ...
            'video game': 'Gaming / PC Gaming / Games',
            # =====================================================================
            # PET SUPPLIES - NEW COMPREHENSIVE MAPPINGS
            # =====================================================================
            'dog food': 'Pet Supplies / Dogs / Food',
            ...
            # =====================================================================
            # GARDEN & OUTDOORS - NEW COMPREHENSIVE MAPPINGS
            # =====================================================================
            'garden hose': 'Garden & Outdoors / Gardening & Lawn Care',
            ...
            # =====================================================================
            # GROCERY / HOUSEHOLD CLEANING - NEW MAPPINGS
            # =====================================================================
            'laundry powder': 'Grocery / Household Cleaning',
            ...
            # =====================================================================
            # BOOKS, STATIONERY & MEDIA - NEW MAPPINGS
            # =====================================================================
            'novel': 'Books, Movies and Music / Fiction / Adult Fiction',
            ...
            # =====================================================================
            # INDUSTRIAL & SCIENTIFIC - NEW MAPPINGS
            # =====================================================================
            'soldering iron': 'Industrial & Scientific / Industrial Electrical',
            'multimeter': 'Industrial & Scientific / Test, Measure & Inspect',
            ...
            # =====================================================================
            # PHONES & TABLETS - EXPANDED MAPPINGS
            # =====================================================================
            ...
            # =====================================================================
            # FASHION - ADDITIONAL EXPANDED MAPPINGS
            # =====================================================================
            'men jeans': "Fashion / Men's Fashion / Clothing / Jeans",
            'men shirt': "Fashion / Men's Fashion / Clothing / Shirts",
            'men shorts': "Fashion / Men's Fashion / Clothing / Shorts",
    ...
    def get_category_for_product(self, product_name, keyword_mapping, categories_list):
        """Match product to best category using priority-based matching"""
        if pd.isna(product_name) or not isinstance(product_name, str):
            return categories_list[0] if categories_list else "Uncategorized"
        ...
        # Check keyword_mapping first (brand and product type mappings)
        for keyword, mapped_category in keyword_mapping.items():
            if keyword in product_lower:
                # Verify the mapped category exists in categories_list
                mapped_lower = mapped_category.lower()
                ...
                # If no exact match, try to find a category that contains the last part of the path
                parts = mapped_lower.split('/')
                last_part = parts[-1].strip() if parts else ''
...
def get_engine() -> CategoryMatcherEngine:
    """Return a module-level singleton engine (no Streamlit cache needed)."""
...
def check_wrong_category(
    ...
    """
    Priority validator for streamlit_app.validate_products().

    Flags products whose assigned category domain (resolved via CATEGORY_CODE
    → full path) doesn't match what the engine predicts from the product NAME.

    Resolution order for the assigned domain:
      1. CATEGORY_CODE column  → code_to_path lookup (most reliable)
      2. CATEGORY column text  → direct top-level split (fallback)
    ...
    Parameters
    ----------
    data              : DataFrame with NAME, CATEGORY and optionally CATEGORY_CODE
    ...
    code_to_path      : Optional dict mapping category_code → Category Path (used to
                        resolve CATEGORY_CODE to a full path for domain comparison)
    """
    ...
    # Build valid domains from real category list
    ...
    stopwords = {
        "and","the","for","with","new","set","pack","pcs","best","top","pro",
        "kit","use","per","all","our","high","quality","inch","size","style",
        "type","mini","large","small","super","ultra","max","plus","big","pair",
    }
    ...
    for _, row in d.iterrows():
        ...
        # Resolve assigned domain — use CATEGORY_CODE → full path when available
        if cat_code and cat_code in code_to_path:
            assigned_full = code_to_path[cat_code]
        else:
            assigned_full = cat_leaf          # fallback: just the leaf name text
        ...
        ) if categories_list else (engine._map_product_type(name) or "")
        if not predicted:
            continue
        ...
        # Both must be real domains in the map
        if assigned_dom not in valid_domains or predicted_dom not in valid_domains:
            continue

        # Skip if assigned is already Miscellaneous (separate legacy check)
        if assigned_dom == "miscellaneous" and predicted_dom == "miscellaneous":
            continue

        # Confidence guard: product name must share a content word with predicted
        # path, OR a direct product-type map hit fires for the predicted domain
        name_words = set(re.findall(r"[a-z]{3,}", name.lower())) - stopwords
        pred_words = set(re.findall(r"[a-z]{3,}", predicted.lower())) - stopwords
        has_overlap = bool(name_words & pred_words)
        direct_map  = engine._map_product_type(name)
        has_direct  = (
            direct_map is not None
            and _top_dom(direct_map) == predicted_dom
        )
